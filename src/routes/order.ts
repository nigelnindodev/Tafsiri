...
import { logger } from "..";
import { unauthorizedIfNotLoggedIn } from "./utils";
            ...
            app
                .get("/", () => OrdersPage, {
                    detail: {
                        ...
                        description:
                            "Return HTMX markup for the main orders page, which by default will load the latest unfinished orders from the /orders/list component",
                ...
                .get(
                    ...
                    },
                    {
                        detail: {
                            ...
                            description:
                                "Returns HTMX markup on clicking of create new order button in the UI. Creates a new order in an initialized state, and if not completed, will be immediately available in the lst of recent unfinished orders",
                            tags: [SwaggerTags.Orders.name],
                        },
                    }
                )
                .get(
                    ...
                    },
                    {
                        detail: {
                            ...
                            description:
                                "Returns HTMX view markup for the latest unfinshed orders. On load, makes a request to /orders/list/all component to fetch the list view markup for unfinshed orders",
                            tags: [SwaggerTags.Orders.name],
                        },
                    }
                )
                .get(
                    ...
                    },
                    {
                        detail: {
                            ...
                            description:
                                "Returns HTMX list view markup for the latest unfinshed orders",
                            tags: [SwaggerTags.Orders.name],
                        },
                    }
                )
                .get(
                    ...
                    },
                    {
                        ...
                        detail: {
                            ...
                            description:
                                "From the list view of unfinshed orders, this endpoint call be called by pressing the resume button, which then loads HTMX markup to resume the order",
                            tags: [SwaggerTags.Orders.name],
                        },
                    }
                )
                .post(
                    ...
                    },
                    {
                        ...
                        detail: {
                            ...
                            description:
                                "Endpoint is called by pressing the confirm button on an order. This returns HTMX success/error markup, which will the load back to the main orders screen after a preconfigured delay",
                            tags: [SwaggerTags.Orders.name],
                        },
                    }
                )
                .post(
                    "/item/updateQuantity/:itemId/:updateType",
                    async (ctx) => {
                    ...
                    },
                    {
                        ...
                        detail: {
                            ...
                            description:
                                "Endpoint is called from the UI during an active order processing. This updates the items quantity and sends a HTMX refresh order event to get the updated counters and total amount due",
                            tags: [SwaggerTags.Orders.name],
                        },
                    }
                )
                .post(
                    "/item/change/:orderId/:inventoryId",
                    async (ctx) => {
                    ...
                    },
                    {
                        ...
                        detail: {
                            ...
                            description:
                                "Endpoint is called from the UI during active order processing, This is updates the items to add or remove an inventory item and the sends a HTMX refresh order event to  get updated order items and amount due",
                            tags: [SwaggerTags.Orders.name],
                        },
                    }
                )
                .post(
                    ...
                    },
                    {
                        ...
                        detail: {
                            ...
                            description:
                                "Updates the payment type for the Order. In the UI, this endpoint is called by selecting the payment type radio button during an active order. Also sends a HTMX refresh order event to update the UI with the correct payment type.",
                            tags: [SwaggerTags.Orders.name],
                        },
                    }
                )
